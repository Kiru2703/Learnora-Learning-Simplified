...
const ChatPage = (() => {
  ...
  function buildPathwayHTML(pathway) {
    const stepsHTML = pathway.steps.map((s, i) => `
      <div class="pathway-step" onclick="openPathway('step-${i}')">
    ...
    if (window.BedrockAI && BedrockAI.isConfigured()) {
      try {
        ...
        if (isPathwayRequest || fileContent) {
          ...
          addMessage('assistant', `${escapeHtml(intro)}<br><br>${buildPathwayHTML(pathway)}`, true);
          addToPathwayList(pathway);
        } else {
        ...
        removeTyping();
        addMessage('assistant', `⚠️ AI error: ${escapeHtml(err.message)}<br><br>Falling back to offline mode.<br><br>${buildPathwayHTML(getPathwayForQuery(query))}`, true);
      }
    } else {
      setTimeout(() => {
        removeTyping();
        ...
        addMessage('assistant', `${responseText}<br><br>${buildPathwayHTML(pathway)}`, true);
        addToPathwayList(pathway);
      }, 1000);
    }
  }
  ...
  function _formatMarkdown(text) {
    ...
    li.innerHTML = `
      ...
      <div>
        <span class="pathway-chapter">${firstStep.topic}</span>
        <span class="pathway-title">${pathway.title.replace(' Pathway','').replace(' Learning','')}</span>
      </div>
    `;
    li.onclick = () => openPathway('custom');
